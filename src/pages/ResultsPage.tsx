...
import GameProgressChart, { GameStats } from "../components/GameProgressChart";
...
function ResultsPage() {
  ...
  const bestScore =
    stats.length > 0 ? Math.max(...stats.map((s) => s.roundsReached)) : rounds;
  const averageScore =
    stats.length > 0
      ? Math.round(
          stats.reduce((sum, s) => sum + s.roundsReached, 0) / stats.length
        )
      : rounds;
